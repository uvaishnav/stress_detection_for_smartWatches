...
class MotionArtifactDetector:
    ...
    def detect_motion_bursts(self, dataset: pd.DataFrame) -> pd.DataFrame:
        ...
        dynamic_threshold = (
            np.median(acc_mag) + 
            1.5*np.std(acc_mag) * (1 + 0.15*dataset['noise_level'].values)  # Reduced from 1.8
        )
        ...
        for i in range(1, len(dataset)):
            ...
            attack_rate = 0.95 / (1 + np.exp(-3*(motion_state[i-1] - 0.4)))  # From 0.9
            decay_rate = 0.1 * (1 - 0.5*motion_state[i-1])  # From 0.15
        ...
        motion_state = np.where(motion_state > 0.6, 1.0, 0.0)  # Reduced from 0.65
